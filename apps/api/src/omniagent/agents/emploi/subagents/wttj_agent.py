"""Agent Welcome to the Jungle : utilise le connecteur plateforme wttj."""
import logging
...
async def run(input_data: dict, user_id: str) -> dict:
    """Recherche d offres sur wttj via le connecteur dedie."""
    criteria = _resolve_criteria(input_data)
    if not criteria or not criteria.get("keywords") and not criteria.get("query"):
        return {"agent": "agent_wttj", "platform": "wttj",
                "offers": [], "status": "no_criteria"}
    connector = connector_manager.get("wttj")
    if connector is None:
        logger.warning("wttj connector not registered")
        return {"agent": "agent_wttj", "platform": "wttj",
    ...
    try:
        ...
        logger.warning("wttj search failed: %s", e)
        return {"agent": "agent_wttj", "platform": "wttj",
