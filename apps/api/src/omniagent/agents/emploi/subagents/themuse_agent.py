"""Agent The Muse : utilise le connecteur plateforme themuse."""
import logging
...
async def run(input_data: dict, user_id: str) -> dict:
    """Recherche d offres sur themuse via le connecteur dedie."""
    criteria = _resolve_criteria(input_data)
    if not criteria or not criteria.get("keywords") and not criteria.get("query"):
        return {"agent": "agent_themuse", "platform": "themuse",
                "offers": [], "status": "no_criteria"}
    connector = connector_manager.get("themuse")
    if connector is None:
        logger.warning("themuse connector not registered")
        return {"agent": "agent_themuse", "platform": "themuse",
